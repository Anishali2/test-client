...
export const MetaAssetsTableDesktop: React.FC<Props> = ({
  ...
  return (
    <div className="flex w-full flex-col rounded-xl bg-primary-light text-white">
        ...
        <table className="w-full table-auto">
          <thead>
            <tr className="text-center">
              ...
              <th className="px-6 py-4 text-sm font-semibold">Owned Offices</th>
              <th className="px-6 py-4 text-sm font-semibold">Actions</th>
            </tr>
          </thead>
          <tbody>
            {metaAssetsDetailList && metaAssetsDetailList.length > 0 ? (
              metaAssetsDetailList.map((item: any) => (
                <tr
                  key={item.id}
                  className="border-b border-[#0E1F30] bg-primary-dark text-center"
                >
