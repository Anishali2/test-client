    ...
    <div className="relative" ref={ref}>
      ...
      >
        <div className="flex items-center justify-between">
          ...
          {isOpen ? (
            <SlArrowUp className="fill-gray-400 size-3" />
          ) : (
            <SlArrowDown className="fill-gray-400 size-3" />
          )}
        </div>
        {error && <p className="mt-1 text-xs text-red-shade-4">{error}</p>}
      </div>
      {isOpen && (
        <div className="custom-scrollbar absolute z-[500] mt-2 max-h-[200px] w-full overflow-y-auto rounded-xl bg-primary text-white shadow-lg">
          {enableFilter && (
            <input
              type="text"
              className="w-full border-b border-gray-shade-3 px-4 py-2 text-primary outline-none"
          ...
          {filteredOptions.length > 0 ? (
            filteredOptions.map((option) => (
              <div
                key={option.value}
                className={clsx(
                  "word-break cursor-pointer rounded-xl px-4 py-2 hover:bg-primary-light",
