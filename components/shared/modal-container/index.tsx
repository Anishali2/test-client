...
const ModalContainer: React.FC<Props> = ({
  ...
  return (
    <ModalPortal wrapperId={modalId}>
      <div
        className={cn(
          "fixed inset-0 z-[2000] flex items-center justify-center overflow-y-auto overflow-x-hidden bg-primary/80 outline-none backdrop-blur-[5.5px] backdrop-filter",
          modalClassName,
        )}
      >
        <div
          ref={modalContentRef}
          className={cn(
            "relative m-4 max-w-xl flex-grow !rounded-xl bg-primary-light px-4 py-5 lg:!p-6",
            modalContentClassName,
          )}
        >
          {children}
